...
class NetworkManager {
    ...
    func fetchFollowers(for user: String, page: Int, completion: @escaping result) {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            guard let serverResponse = response as? HTTPURLResponse, serverResponse.statusCode == 200
                else { completion(.failure(.unexpectedStatusCode)); return }
            ...
            } catch { completion(.failure(.failedJSONParsing)) }
        }; task.resume()
    }
    ...
    func fetchAvatars(from StringUrl: String, completion: @escaping (UIImage) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: imageURL) { (data, response, error) in
            ...
            if let image = UIImage(data: receivedData) { completion(image) }
        }; task.resume()
    }
    ...
    /// - Parameters:
    ///   - follower: name of the GitHubFollower
